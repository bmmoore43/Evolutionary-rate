...
class tree_utility:
	...
	def rename(self,tree,name,nhx=0,nflag=0,call=0):
		...
		if call:
			tline = tree
			ndict = name
			# If nflag is 1, value has old name, convert it to key.
			if nflag == 0:
				ndict2 = {}
				for i in ndict:
					ndict2[ndict[i]] = i
		...
		while c < len(tline):
			...
			if full_name:
				found = 0
				nlist = name.split(":")
				#print "1:",nlist[0],ndict.has_key(nlist[0])
	...
	#
	# Get subtree based on an outgroup. Need to simplify tree first.
	#
	...
	#
	# Get subtree based on the taxa within the subtree. Note that:
	# * Assume at least the left and right most taxa are included
	#
	def get_subtree2(self,tree,taxa):
		# Read tree into a string
		inp = open(tree)
		tre = inp.readlines()
		
		if len(tre) > 1:
			print "Convert tree to oneline format..."
			tre = self.one_line(tree,0,1)
		else:
			tre = tre[0]
		tre = tre.strip()
		...
		#print ">>>>"
		#print tre
		# Read taxa file into a list
		inp = open(taxa)
		tax = inp.readlines()
		#print tax
		# Find out the left and right most indices in the tree string where taxa
		# names can be found
		iL = len(tre) # left most
		iR = -1       # right most
		for i in tax:
			i    = i.strip()
			iidx = tre.find(i)
			if iidx == -1:
				print "ERR: %s not in %s, quit..." % (i,tree)
				sys.exit(0)
			else:
				if iidx < iL:
					iL = iidx
		...
		# Get all "(", ")" indices
		LPI = self.allindices(tre,"(",[],0) # Left parenthesis indicies
		RPI = self.allindices(tre,")",[],0) # Right
		CMI = self.allindices(tre,",",[],0) # Comma
		
		# Go through the subtree part of the tree and expand on left direction
		sL = iL # subtree left
		sR = iR # subtree right
		#print LPI
		#print "X:",tre[iL:iR+10]
		unmatchLB = 0
		for i in range(iL,iR+1):
			...
			if tre[i] == "(":
				...
				# Only expand left and right if LB is not found before
				# This "," has a corresponding "(". Reset foundLB to 0
				if unmatchLB > 0:
					unmatchLB -= 1
				# This "," does not have zero unmatched "(", expand to left "(".
				else:
					iLB = bisect(LPI,sL)-1 # Index in a list of "(" indices.
					#print " sL,new_sL:",sL,LPI[iLB]
					# Update sL, note that -1 here so the index is not right on the same
					# "(", otherwise, it will keep finding the same thing.
					sL = LPI[iLB]-1
					#print tre[sL+1:sR]
			#print " unmatchLB:",unmatchLB
		#print "Y:",tre[sL+1:sR]
		
		sL2 = sL+1
		# Now check the new subtree and expand on the right
		cLB = tre[sL2:sR].count("(") # count "(" in the left corrected subtree
		cRB = tre[sL2:sR].count(")") # count ")"
		rRB = tre[:sR].rfind(")")    # index of the R-most ")" in the subtree
		irRB= bisect(RPI,rRB)        # index of the R-most ")" in the ")" indices
		# cLB-cRB tell us how many ")" is missing. irRB-1 because the rRB will be
		# exact the same as one of the element in the ")" indices so the bisect
		# result will be +1.
		sR2 = RPI[irRB-1+(cLB-cRB)]  # index of the new R-most ")"
		#print "Z:",tre[sL2:sR2+1]
		
		# Find the closest right comma or right bracket, whichever comes first
		irCM = bisect(CMI,sR2+1)
		irRB = bisect(RPI,sR2+1)
		err = 0
		complete = 0
		if irCM == len(CMI):    # already last comma
			if irRB == len(RPI):
				#print "ERR: comma and right bracket both do not qualify!"
				#print [tre]
				#print "RPI:",len(RPI),RPI
				#print "CMI:",len(CMI),CMI
				#sys.exit(0)
				#err = 1
				# If this happens, the substree already complete.
				complete = 1
			else:
				sR3  = RPI[irRB]
		elif irRB == len(RPI):  # already last right ")"
			sR3  = CMI[irCM]
		else:
			crCM = CMI[irCM]
			crRB = RPI[irRB]
			if CMI[irCM] > RPI[irRB]:
				sR3  = RPI[irRB]
			else:
				sR3  = CMI[irCM]
		...
		if complete:
			...
			subT = tre[sL2:sR3]
		#print subT
		# The subtree has an "," added to the end.
		subT += ","
		
		oup = open("subtree_%s_%s" % (tree,taxa),"w")
		oup.write(subT+"\n")
		oup.close()

	# @param tree  A file with multiple trees, each tree is in one line
	# @param taxa  A file with taxa subtree to get, each subtree taxa in one line
	#              seprated by ",". The tree and taxa files should be exactly the
	#              same order.
	# @param outname output file name
	def batch_sub2(self,tree,taxa,outname):
		if outname == "":
			outname = "subtrees"
		TRE = open(tree).readlines()
		TAX = open(taxa).readlines()
		for i in range(len(TRE)):
			...
			oup.write(tax)
			oup.close()
			self.get_subtree2("TMP_tre","TMP_tax")
			os.system("cat subtree_TMP_tre_TMP_tax >> %s" % outname)
			
		os.system("rm *TMP_*")	
		
	# From http://code.activestate.com/recipes/
	# 499314-find-all-indices-of-a-substring-in-a-given-string/
	def allindices(self,string, sub, listindex, offset):
	...
	def get_taxa(self,tree,taxa1,taxa2,call=0):
	...
	#
	# Get subtrees with the organization (xxxx,each_name);
	...
	def parse_notung(self,tree,batch=0):
		...
		if batch:
			...
			oup.write("NodeTaxa\tDup(1)/Spe(0)\tLeft\tRight\n")
			for i in og:
				#print i
				for j in og[i]:
					#print "",j
	...
	def help(self):
		...
		print "    batch_sub2 - use get_subtree2, NEED: t, n, OPT: o"
		...
		print "    subtree - get subtree based on outgroup, NEED: t,x, OPT: o"
		print "    subtree2 - get subtree based on ingroup taxa, NEED: t,x"
		...
		print "      depend on the nflag setting. "
		print "      For batch_sub: a list of names."
		print "      for batch_sub2: each line with subtree taxa seprated by ','"
		print "      For replace_sub: [name][subtree]"
		...
		print " -o  output file name. For subtree, if a string is not specified,"
		print "     subtree string will be printed and returned"
...
if __name__ == '__main__':
	...
	elif function == "subtree2":
		if tree == "" or taxa == "":
			print "\nNeed tree file and taxa file\n"
			utility.help()
		else:
			utility.get_subtree2(tree,taxa)
	...
	elif function == "batch_sub2":
		if tree == "" or name == "":
			print "\nNeed tree and name files\n"
			utility.help()
		else:
			utility.batch_sub2(tree,name,outname)
