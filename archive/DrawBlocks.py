...
import SVGWriter,FileUtility,sys,FastaManager
...
class draw_blocks:
	...
	def draw_chr2(self,dat_table,dup):
		...
		# cdict = {chr:{L_coord:[gene_id,gene_north,gene_south}}
		cdict  = {}
		Ylen   = 0
	...
	def draw_match(self,c1,c2,blast,idT,lenT,Yzoom,FLAG=0):
		...
		writer.set_font_size(8)
		if chr1S > 1e6:
			S = str(float(chr1S)/1e6)
			writer.draw_string("%sMb" % S[:S.find(".")+3],Xoff,Yoff-10)
		else:
			writer.draw_string("%i" % chr1S,Xoff,Yoff-10)
		if chr2S > 1e6:
			S = str(float(chr2S)/1e6)	
			writer.draw_string("%sMb" % S[:S.find(".")+3],Xoff*3,Yoff-10)
		else:
			writer.draw_string("%i" % chr2S,Xoff*3,Yoff-10)
		if chr1E > 1e6:
			S = str(float(chr1E)/1e6)	
			writer.draw_string("%sMb" % S[:S.find(".")+3],Xoff,
											float(chr1E-chr1S)/Yzoom+Yoff+20)
		else:
			writer.draw_string("%i" % chr1E,Xoff,
											float(chr1E-chr1S)/Yzoom+Yoff+20)
		if chr2E > 1e6:
			S = str(float(chr2E)/1e6)	
			writer.draw_string("%sMb" % S[:S.find(".")+3],Xoff*3,
											float(chr2E-chr2S)/Yzoom+Yoff+20)
		else:
			writer.draw_string("%i" % chr2E,Xoff*3,
											float(chr2E-chr2S)/Yzoom+Yoff+20)
					
		writer.set_fill_c("silver")
		...
		#              1: exon 
		def draw_feature(flist,order,ftype=0,idx=0):
			...
			if order == 1:
				...
				x = Xoff*3 + chrW*2
				r = Xoff*3 + 30
			yL = float((flist[0]-s)/Yzoom)+Yoff
			yR = float((flist[1]-s)/Yzoom)+Yoff
			if ftype == 0:
				writer.set_stroke_w(1.5)
				writer.set_stroke_c("black")
				#writer.draw_rect(x-chrW/2,yL,chrW,yR-yL)
				writer.draw_line(x,yL,x,yR)
				writer.set_stroke_w(0.5)
				if flist[2] == "-1" or flist[2] == "-":
					y20 = yL+0.2*(yR-yL)
					writer.draw_line(x-chrW/2,y20,x,yL)
					writer.draw_line(x+chrW/2,y20,x,yL)
					#writer.draw_triangle(x-chrW/2,yL,x,yL,x-chrW/2,yL+(yR-yL)/10)
					#writer.draw_triangle(x+chrW/2,yL,x,yL,x+chrW/2,yL+(yR-yL)/10)
				elif flist[2] == "1" or flist[2] == "+":
					y20 = yR-0.2*(yR-yL)					
					writer.draw_line(x-chrW/2,y20,x,yR)
					writer.draw_line(x+chrW/2,y20,x,yR)
				...
				writer.set_stroke_c("darkred")
				writer.draw_line(x,yL+(yR-yL)/2,r,20+idx*10)
				writer.set_stroke_w(0)
				writer.draw_string(flist[3],r,20+idx*10)
			elif ftype == 1:
				writer.set_fill_c("lightgreen")
				writer.draw_rect(x-chrW/2,yL,chrW,yR-yL)
		...
		writer.set_stroke_c("black")
		writer.set_fill_c("white")
		writer.set_font_size(7)

		# draw gene
		
		# 03/04/2010
		# put the genes into a dict: {L:[R,ori,genename]}
		def ltod(genes):
			gdict = {}
			for j in genes:
				L = int(j[0])
				...
				if L not in gdict:
					...
					print "Redun L coord:",L,gdict[L][2],N
			return gdict
		...
		for j in gk:
			C = [j]
			C.extend(gdict[j]) # [L,R,O,gene]
			draw_feature(C,1,0,idx)
			idx += 1
		...
		def draw_pair(mlist):
			...
			writer.set_stroke_w(0.25)
			# Draw polyline
			yL1 = float(mlist[1])/Yzoom + Yoff
			yR1 = float(mlist[2])/Yzoom + Yoff
			yL2 = float(mlist[3])/Yzoom + Yoff
			yR2 = float(mlist[4])/Yzoom + Yoff
			x1  = Xoff++chrW*3/2
			x2  = Xoff*3-chrW/2

			writer.draw_polyline("%f,%f %f,%f %f,%f %f,%f" % \
							(x1,yL1,x2,yL2,x2,yR2,x1,yR1),0.5,c)
	...
	#
	# gdom file is tab-delimited like:
	#  seq_name dom1_start dom1_name|length dom2... seq_len
	# e.g.
	#  At5g38990	1	SIGNAL|16	441	TRANS|23	525	STYKc|275	880
	# 
	# @param dom    4 column tab-delim with seq_name,dom_name,L,R
	# @param fasta  Fasta file for the sequences in dom_coord.
	#
	def make_gdom(self,dom,fasta):
		# get sequence length
		fm    = FastaManager.fasta_manager()
		fdict = fm.fasta_to_dict(fasta)
	...
	def help(self):
		...
		print "         make_gdom - Create gdom file, NEED: dom, fasta"
		...
		print " -dom    Four column file with seq_name,dom_name,L,R"
		print " -fasta  Fasta sequence"
...
if __name__ == '__main__':
	...
	function = matrix  = scheme = R = gdom = coords = dat = name = dup = \
			   c1 = c2 = blast = csize = cname = cleng = dom = fasta = "" 
	X = Y = 1.0
	A = 3.0
	H = 20
	Yinc = 30
	...
	elif function == "make_gdom":
		if dom == "" or fasta == "":
			print "\nNeed dom and fasta files.\n"
			print " -h for help"
			sys.exit(0)
		dblocks.make_gdom(dom,fasta)
