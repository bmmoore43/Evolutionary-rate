...
class svg_writer:
	...
	def draw_polyline(self,cstr,
			fill_opacity=1.0,fill_c="white",stroke_c="black",stroke_w=0.5):
				
		self.oup.write(
			"\t<polyline style=\"fill_opacity:%f; fill:%s; stroke:%s; stroke-width:%f\" " % \
			(fill_opacity,self.fill_c,self.stroke_c,stroke_w) + \
			"points=\"%s\" />\n" % cstr)
