...
class aln_util:
	...
	def batch_tree(self,group,fasta,clus_dir,bootstrap=0,isdir=0):
		...
		for i in glist:
			...
			os.system("clustalw2 %s.fa " % (clus_dir,i) + ">> TMP.log")
			
			print " tree :",i
			if bootstrap != 0:
				os.system("clustalw2 %s.aln /BOOTSTRAP=400 /BOOTLABELS=node" % \
					(i) + ">> TMP.log")
			else:
				os.system("clustalw2 %s.aln -tree" % \
					(i) + ">> TMP.log")
	...
	def batch_tree2(self,groups,fasta,c_dir,w_dir="./",mode=1,spflag=1,pick="",
		...
		for i in gdict.keys():
			...
			if(os.path.isfile("%s/aln/%s.aln" % (w_dir,i)) and \
				...
				print " align..."
				if aln_profile == "":
					os.system("clustalw2 %s/seq/%s.fa" % \
						   		(w_dir,i) + ">>%s/TMP_%s.log" % (w_dir,i))
				else:
					p1 = aln_profile
					#print "%sclustalw -sequences -PROFILE1=%s/%s "%(c_dir,w_dir,p1)+\
					#          "-PROFILE2=%s/seq/%s.fa "     %(w_dir,i)       +\
					#          ">> %s/TMP_%s.log" %(w_dir,i)
					os.system("clustalw2 -sequences -PROFILE1=%s/%s "%(w_dir,p1)+\
					          "-PROFILE2=%s/seq/%s.fa "     %(w_dir,i)       +\
					          ">> TMP_%s.log" %i)
			...
			if mode:
				if((os.path.isfile("%s/tre/%s.ph"  % (w_dir,i))       and \
				   ...
				   (os.path.isfile("%s/tre/%s.phb" % (w_dir,i))       and \
					...
					print " build tree..."
					if bootstrap == 0:
						os.system("clustalw2 %s/aln/%s.aln -tree >> %s/TMP_%s.log" % \
								  (w_dir,i,w_dir,i))
					else:
						os.system("clustalw2 aln/%s.aln"       %(i) +\
	...
	def batch_align(self,groups,fasta,clus_dir,paml_dir):
		...
		for i in gdict.keys():
			...
			print "clustalw2 /INFILE=TMP2.FA /OUTFILE=TMP2.GDE /OUTPUT=GDE"
			os.system("clustalw2 /INFILE=TMP2.FA /OUTFILE=TMP2.GDE /OUTPUT=GDE")
	...
	def parse_gap(self,pairs,fasta,clus_dir):
		...
		for i in inlines:
			...
			# align
			error = self.do_clustal(L[0],L[1],pdict,"TMP")
	...
	def rate_pair(self,pep,cds,pairs,clus_dir,prog,paml_dir,window,step,
		...
		for i in inlines:
			...
			if len(llist[0]) > 30 or len(llist[1]) > 30:
				...
				continue
			error = self.do_clustal(llist[0],llist[1],pdict,"TMP")
			if error:
				print "   sequence absent"
				oup_rate.write("%s\t%s\t" % (llist[0],llist[1]))
				oup_rate.write("-\t-\t-\t-\tsequence_absent\n")
	...
	##
	def do_clustal(self,id1,id2,fdict,outheader):
		...
		os.system("clustalw2 %s.FA /OUTFILE=%s.GDE /OUTPUT=GDE >> run.log" % \
				   (outheader,outheader))
		#sys.exit(0)
		return error
